...
import useWindowWidth from "./hooks/windowWidthHook"; // Import the custom hook
...
export default function MainContent() {
    ...
    return (
        <>
            ...
            <div className={`main_container flex top-15 ${ width > 770 ? "left-[3.75rem]" : "left-0" } fixed ${ width > 770 ? "w-[calc(100vw-3.75rem)]" : "w-full" } h-[calc(100vh-3.75rem)] ${darkMode ? "bg-gray-800" : "bg-white"}`}>
                {/* Sidebar Navigation */}
                ...
                {/* Main Content Area */}
                <div className={`main_content_container h-[calc(100vh-3.75rem)] top-15 overflow-y-auto fixed ${sidebarOpen ? "w-[calc(100vw-20rem)] left-[20rem]" : `${ width > 770 ? "w-[calc(100vw-3.75rem)]" : "w-full" } ${ width > 770 ? "left-[3.75rem]" : "left-0" }`}`}>
                    { width > 770 ? (
                        <Routes>
                        ...
                        <>
                            <Home pageLoaded = {!loading} />
                            <About pageLoaded = {!loading} />
                            <Project pageLoaded = {!loading} />
                        </>
                    )}
                </div>
            </div>
